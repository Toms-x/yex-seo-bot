...
comes from Claude reading the signal in context and outputting an actionable
...
from config.settings import (
    ANTHROPIC_API_KEY, CLAUDE_MODEL, YEX_EDITORIAL_CONTEXT
)
...
{{
  ...
  "angle": "..."
    ...
    try:
        ...
        log.error("Claude returned invalid JSON: %s | text: %s", e, text[:200])
        return {"skip": True}
    except Exception as e:
        log.error("Claude interpretation failed: %s", e)
        return {"skip": True}
